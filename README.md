...
| Geo-matching | Haversine formula (pure Python) | No dependencies, exact for this scale |
...
**Why rule-based scoring instead of ML?** The scoring logic needs to be explainable to a procurement manager who will act on it. A weighted rule model can be audited, adjusted, and justified. A trained classifier cannot.
